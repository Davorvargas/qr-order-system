...
export default function MenuPage() {
  ...
  const [allCategories, setAllCategories] = useState<Category[]>([]); // <-- Restaurado a allCategories
  ...
  useEffect(() => {
    ...
    const getMenuData = async () => {
      ...
      if (categoriesData) setAllCategories(categoriesData); // <-- Restaurado
  ...
  // 3. Restaurar y corregir el useMemo
    ...
    <>
      ...
      >
        ...
        <OrderForm
          categories={availableCategories}
          items={allMenuItems}
          tableId={tableId} // Pasamos el ID único de la mesa, no el número
        />
      </main>
    </>
  );
