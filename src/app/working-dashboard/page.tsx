...
export default function WorkingDashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      if (userError || !user) {
        console.log('❌ No user, redirecting to working login')
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="bg-white shadow-sm">
        <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold">🎉 Working Dashboard!</h1>
            <p className="text-gray-600">
              {user?.email} | {profile?.restaurants?.name || 'Sin restaurante'}
            </p>
            <p className="text-sm text-green-600">✅ RLS + Auth funcionando sin middleware</p>
          </div>
          <button
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
                ...
                {orders.map((order) => (
                  <div key={order.id} className="border-l-4 border-blue-500 pl-3 py-2">
                    ...
                    <p className="text-sm text-gray-600">
                      Bs {order.total_price} | {order.status}
                    </p>
                  </div>
                ))}
              </div>
        ...
        <div className="mt-6 bg-green-50 border border-green-200 p-4 rounded">
          ...
          <p className="text-green-700 text-sm mt-1">
            RLS filtra automáticamente por restaurante. Login/logout funciona con window.location.
          </p>
        </div>
      </div>
    </div>
