...
INCLUYE verificación de activación de impresora.
"""
...
# ID del restaurante para verificar activación
...
def check_printer_active():
    """Verificar si la impresora está activa en la base de datos"""
    global supabase
    if not supabase:
    ...
    try:
        ...
        if not is_active:
            print("[INFO] IMPRESORA DESACTIVADA - No se procesarán pedidos")
        return is_active
...
def process_new_orders():
    """Procesa nuevas órdenes con verificación de activación"""
    global supabase
    if not supabase:
        supabase = load_environment()
    ...
    while running:
        try:
            # VERIFICAR SI LA IMPRESORA ESTÁ ACTIVA ANTES DE PROCESAR
            if not check_printer_active():
                print("[INFO] Impresora desactivada, esperando...", end='\r')
                time.sleep(10)
                continue
...
def main():
    ...
    print("[INFO] CON verificación de activación de impresora")
    
    # Cargar configuración
    supabase = load_environment()
    print("[OK] Conectado a Supabase")
    
    # Probar impresora física
    if not test_printer():
        print("[ERROR] No se puede continuar sin impresora funcionando")
        sys.exit(1)
    
    # Verificar estado inicial de la impresora en BD
    if check_printer_active():
        print("[OK] Impresora activa en base de datos")
    else:
        print("[WARN] Impresora desactivada en base de datos - esperando activación")
